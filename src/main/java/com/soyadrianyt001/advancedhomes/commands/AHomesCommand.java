...
public class AHomesCommand implements CommandExecutor, TabCompleter {

    private final AdvancedHomes plugin;
    public AHomesCommand(AdvancedHomes plugin) { this.plugin = plugin; }
        ...
        switch (args[0].toLowerCase()) {
            ...
            case "info" -> {
                ...
                if (sender instanceof Player player) {
                    ...
                    sender.sendMessage(msg.getRaw("info-homes")
                            .replace("{count}", String.valueOf(count))
                            .replace("{limit}", limit == -1 ? "∞" : String.valueOf(limit)));
                }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.GOLD + "" + ChatColor.STRIKETHROUGH + "----------------------------");
        sender.sendMessage(ChatColor.YELLOW + " /ahome " + ChatColor.GRAY + "[nombre] - Ir a un hogar");
        sender.sendMessage(ChatColor.YELLOW + " /ahome create " + ChatColor.GRAY + "[nombre] - Crear hogar");
        sender.sendMessage(ChatColor.YELLOW + " /ahome remove " + ChatColor.GRAY + "<nombre> - Eliminar hogar");
        sender.sendMessage(ChatColor.YELLOW + " /ahome list " + ChatColor.GRAY + "- Ver tus hogares");
        sender.sendMessage(ChatColor.YELLOW + " /ahome gui " + ChatColor.GRAY + "- Abrir menu");
        sender.sendMessage(ChatColor.YELLOW + " /ahomes reload " + ChatColor.GRAY + "- Recargar plugin");
        sender.sendMessage(ChatColor.YELLOW + " /ahomes reset " + ChatColor.GRAY + "<jugador> - Resetear hogares");
        sender.sendMessage(ChatColor.GOLD + "" + ChatColor.STRIKETHROUGH + "----------------------------");
    }
